...
async def var_set(manager: Manager, message: Message):
    if message.get('Variable', '') != 'MIXMONITOR_FILENAME':
        return
    await Channel.filter(id=message.get('Uniqueid', '')).update(monitor_file_name=message.get('Value'))


async def new_channel(manager: Manager, message: Message):
    from_number = message.get('CallerIDNum')
    request_number = message.get('Exten')
    call = None

    if from_number and request_number:
